...
namespace Snack_Sniker.Models
{
    public class CRU_GenderMV
    {
        ...
        {
            GetAllGenders();
            var editgender = new PizzaAndDrinkDbEntities1().GenderTables.Where(g => g.GenderID == id).FirstOrDefault();
            if (editgender != null)
            {
        ...
        private void GetAllGenders()
        {
            ...
            foreach (var gender in new PizzaAndDrinkDbEntities1().GenderTables.ToList())
            {
